...
class sFFS: 
    def __init__(self, classifier, metric, n_channels, n_jobs=1, verbose=0):
    ...
    def fit(self, X, y):
        X_copy = X.copy()  # to improve performance
        selected_channels = []
        remaining_channels = list(range(X_copy.shape[1]))
        best_score = -np.inf
        no_improvement_count = 0 

        with tqdm(total=self.n_channels, disable=not self.verbose) as pbar:
            while len(selected_channels) < self.n_channels and remaining_channels:
                if self.verbose:
                    print(f"Iteration {len(selected_channels) + 1}")
                    print(f"Remaining channels: {len(remaining_channels)}")

                try:
                    results = Parallel(n_jobs=self.n_jobs)(
                        delayed(self._evaluate_channel)(X_copy, y, selected_channels + [ch])
                        for ch in remaining_channels
                    )
                    if results:
                        best_new_channel, best_new_score = max(results, key=lambda x: x[1])
                    else:
                        best_new_channel = None
                        best_new_score = -np.inf
                except Exception as e:
                    print(f"Error during forward step: {e}")
                    break

                if best_new_channel is not None:
                    selected_channels.append(best_new_channel)
                    remaining_channels.remove(best_new_channel)
                ...
                if len(selected_channels) > 1:
                    try:
                        results = Parallel(n_jobs=self.n_jobs)(
                            delayed(self._evaluate_channel)(X_copy, y, [ch for ch in selected_channels if ch != remove_ch])
                            for remove_ch in selected_channels
                        )
                        if results:
                            best_remove_channel, best_remove_score = max(results, key=lambda x: x[1])
                        else:
                            best_remove_channel = None
                            best_remove_score = -np.inf
                    except Exception as e:
                        print(f"Error during floating step: {e}")
                        break

                    if best_remove_channel is not None and best_remove_score > best_score:
                        selected_channels.remove(best_remove_channel)
                        remaining_channels.append(best_remove_channel)
                ...
                pbar.update(1)
                pbar.set_description(f"Selected {len(selected_channels)} channels, Best score: {best_score:.4f}")

            
                if no_improvement_count >= 5:
                    print("No improvement in the last few iterations, stopping.")
                    break
        ...
        if self.verbose:
            print(f"Final selected channels: {self.selected_channels}")
            print(f"Final best score: {self.best_score_:.4f}")

        return self
      
    def _evaluate_channel(self, X, y, temp_selected_channels):
        clf = clone(self.classifier)
        score = cross_val_score(clf, X[:, temp_selected_channels], y, cv=5, scoring=self.metric).mean()
        return temp_selected_channels[-1], score
